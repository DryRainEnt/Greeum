...
from greeum import __version__ as GREEUM_VERSION
...
class UnifiedBaselineTracker:
    ...
    def generate_report(self, performance_data: Optional[Dict[str, Any]] = None) -> str:
        ...
        report = f"""# Greeum v{GREEUM_VERSION} 통합 성능 기준점 리포트

## 측정 정보
- **측정 시간**: {performance_data.get('measurement_timestamp', 'N/A')}
- **Greeum 버전**: {performance_data.get('greeum_version', GREEUM_VERSION)}
- **측정 모드**: {mode.upper()}
...
---
*Generated by UnifiedBaselineTracker v{GREEUM_VERSION}*
"""
...
def main():
    ...
    logger.info(f"Greeum v{GREEUM_VERSION} 통합 기준점 측정 시작 - 모드: {args.mode}")
